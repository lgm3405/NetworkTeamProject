...
{
    private GameObject cameraPlayer;   // 바라볼 플레이어 오브젝트
    ...
    void Start()
    {
        cameraPlayer = GameObject.Find("Player(Clone)");
    }
    ...
    void Update()
    {
        ...
        transform.position = cameraPlayer.transform.position - transform.rotation * reverseDistance;   // 플레이어 위치에서 카메라가 바라보는 방향에 벡터값을 적용한 상대 좌표를 차감
    }
}
